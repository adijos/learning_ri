...
def extract_words(text_path, length=8):
    """ returns words defined by characters of length <=8 separated by spaces """
    text = load_text_spaces(text_path)
    words = text.split()
    for i in xrange(len(words)):
        old_word = words[i]
        if len(old_word) > 8:
            words[i] = old_word[0:8]
    return words
